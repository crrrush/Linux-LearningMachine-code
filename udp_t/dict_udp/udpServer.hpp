...
using func_t = std::function<void(std::string &, const std::string &, const std::string &, const uint16_t &)>;
    ...
    void start()
    {
        ...
        while (true)
        {
            ...
            if (n > 0)
            {
                ...
                _callback(result, buffer, ip, port);
            }

            if (-1 == sendto(_socketfd, result.c_str(), result.size(), 0, (const struct sockaddr *)&sa, sizeof(sa)))
            {
